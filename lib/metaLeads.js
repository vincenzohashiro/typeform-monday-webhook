...
export async function createMondayItemFromLead(leadData) {
  ...
  for (const field of leadData.field_data ?? []) {
    ...
    if (mapping && value) {
      colValues[mapping.colId] = formatValue(mapping.type, value);
    }
  }
  ...
  const json = await res.json();
  if (json.errors?.length) throw Object.assign(new Error(json.errors[0].message), { code: "API_ERROR" });
...
function formatValue(type, value) {
  switch (type) {
    ...
    case "phone": return { phone: value, countryShortName: "DK" };
