...
export async function createMondayItemFromLead(leadData) {
  ...
  // Diagnostic: verify the API key can see this board
  const boardCheck = await fetch("https://api.monday.com/v2", {
    method:  "POST",
    headers: { "Content-Type": "application/json", Authorization: token, "API-Version": "2024-01" },
    body: JSON.stringify({ query: `query { boards(ids: [${BOARD_ID}]) { id name } }` }),
  });
  const boardJson = await boardCheck.json();
  console.log("Board access check:", JSON.stringify(boardJson));
  ...
  const res  = await fetch("https://api.monday.com/v2", {
    ...
    body: JSON.stringify({
      ...
      variables: hasColumns
        ? { boardId: BOARD_ID, name: itemName, cv: JSON.stringify(colValues) }
        : { boardId: BOARD_ID, name: itemName },
    }),
  });
