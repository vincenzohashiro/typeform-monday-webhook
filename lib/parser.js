...
const REF_MAP = {
  ...
  "35f3c8c3-db0e-44e8-acde-d5f6fe9a7d3f": { id: "dropdown_mm2zgbbf",    type: "dropdown",                       label: "Tillægspriser"           },
...
// ── Date/time parsing ─────────────────────────────────────────────────────────
const DANISH_MONTHS = {
  januar: "01", february: "02", februar: "02", marts: "03", march: "03",
  april: "04", maj: "05", may: "05", juni: "06", june: "06",
  juli: "07", july: "07", august: "08", september: "09", sept: "09",
...
function parseDateValue(raw) {
  ...
  // Danish format: "D.2 september 17:10" or "2. september 17:10" or "2 september 2026 17:10"
  const dk = s.match(/^(?:d\.?\s*)?(\d{1,2})\.?\s+([a-zæøå]+)(?:\s+(\d{4}))?(?:\s+(\d{1,2})[\.\:](\d{2}))?/i);
  if (dk) {
    const monthNum = DANISH_MONTHS[dk[2].toLowerCase()];
    if (monthNum) {
      const year = dk[3] ?? String(new Date().getFullYear());
      const dateStr = `${year}-${monthNum}-${dk[1].padStart(2, "0")}`;
      const timeStr = dk[4] ? `${dk[4].padStart(2, "0")}:${dk[5]}` : null;
...
function formatForMonday(mapping, val) {
  ...
  switch (mapping.type) {
    ...
    case "dropdown": return { labels: s.split(",").map(l => l.trim()).filter(Boolean) };
    case "date": {
