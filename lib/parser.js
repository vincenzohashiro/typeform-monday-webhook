...
// ── Timezone: Turkish (UTC+3) → UTC for Monday storage ───────────────────────
// Monday stores time as UTC internally and displays in the viewer's timezone.
// So we convert TR → UTC. Danish staff (UTC+2 CEST) will see the correct DK time.
// e.g. 12:34 TR (UTC+3) → 09:34 UTC stored → Danish viewer sees 11:34 DK ✓
function convertTRtoDK(dateStr, timeStr) {
  ...
  const utc = tr.toUTC();
  return {
    date:  utc.toISODate(),          // UTC date (handles midnight edge cases)
    time:  utc.toFormat("HH:mm:ss"), // UTC time for Monday to store
...
function mapFields(fields) {
  ...
  return { colValues, searchValue, name, timeLabels, warnings };
}
