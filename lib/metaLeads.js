...
const FIELD_MAP = {
  // "email":        { colId: "email_mkwx219b",   type: "email", label: "Email"       }, // temporarily disabled — column type mismatch
...
export async function createMondayItemFromLead(leadData) {
  ...
  // Mark source as Meta — temporarily disabled for isolation test
  // colValues["color_mkwx19y8"] = { label: "Meta" };

  // Fetch the first group ID — create_item needs it on some boards
  const groupRes  = await fetch("https://api.monday.com/v2", {
    method:  "POST",
    headers: { "Content-Type": "application/json", Authorization: token, "API-Version": "2024-01" },
    body: JSON.stringify({ query: `query { boards(ids: [${BOARD_ID}]) { groups { id title } } }` }),
  });
  const groupJson = await groupRes.json();
  const groupId   = groupJson.data?.boards?.[0]?.groups?.[0]?.id ?? null;
  console.log("Groups:", JSON.stringify(groupJson.data?.boards?.[0]?.groups), "using:", groupId);
    ...
    body: JSON.stringify({
      query: hasColumns
        ? `mutation ($boardId: ID!, $groupId: String!, $name: String!, $cv: JSON!) {
             create_item(board_id: $boardId, group_id: $groupId, item_name: $name, column_values: $cv) { id name }
           }`
      ...
      variables: hasColumns
        ? { boardId: BOARD_ID, groupId, name: itemName, cv: JSON.stringify(colValues) }
        : { boardId: BOARD_ID, groupId, name: itemName },
    }),
  });
